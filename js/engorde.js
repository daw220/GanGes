...
function anadir() {
    ...
    $.ajax({
        type: "POST",
        url: "../operaciones/engordeOperations.php",
        data: form,
        contentType: false,
        processData: false,
    ...
    for (let i = 0; i < json.length; i++) {
        ...
        for (let j = 0; j < keys.length; j++) {
            ...
            if(j == 4)
            {
                ...
                span.innerHTML=`Lleva ${parseInt(json[i][keys[j]])} dias.`
...
function editar(id) {
    borrarInput();
    if (id != 0){
        $.get("../operaciones/engordeOperations.php?accion=1&id="+id, (data) => {
...
function inicio() {
    ...
    $.get("../operaciones/engordeOperations.php?accion=0",function (data) {
            tabla(["CROTAL","NOMBRE","ESTADO","TIEMPO"], JSON.parse(data));
        }
    );

    $.get("../operaciones/engordeOperations.php?accion=3", (data) => {
